...
function demoProvider(index: number, overrides: Partial<AdminProvider> = {}): AdminProvider {
  ...
  return {
    ...
    verification_status: 'verified', verification_status_label: '已认证',
    ...
    service_city_code: '130400', service_city_name: '邯郸市', max_service_radius_km: 20,
...
async function load() {
  ...
  try {
    ...
    const data = await adminApi.managedProviders({
      ...
      verification_status: verificationFilter.value, accepting: acceptingFilter.value,
...
function resetFilters() {
  search.value = ''; cityFilter.value = ''; statusFilter.value = ''; verificationFilter.value = ''; acceptingFilter.value = 'all'; page.value = 1; load()
}
