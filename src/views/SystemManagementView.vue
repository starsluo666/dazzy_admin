...
const selectedMemberPermissionGroups = computed(() => permissionGroups.value.map((group) => ({
  ...group,
  selected: group.permissions.filter((permission) => selectedMemberRole.value?.permissions.includes(permission.code)),
    ...
    <el-drawer v-model="memberDrawerVisible" size="500px" :with-header="false" destroy-on-close>
      <div class="system-drawer">
        ...
        <el-form class="drawer-form" label-position="top">
          ...
          <div v-if="selectedMemberRole" class="permission-preview-card"><header><div><strong>权限预览</strong><span>{{ selectedMemberRole.name }} · {{ selectedMemberRole.permissions.length }} 项功能权限</span></div><el-tag size="small" :type="selectedMemberRole.is_system ? 'info' : 'primary'" effect="plain">{{ selectedMemberRole.is_system ? '系统内置' : '自定义' }}</el-tag></header><div class="permission-preview-groups"><span v-for="group in selectedMemberPermissionGroups" :key="group.key"><b>{{ group.label }}</b>{{ group.selected.map((permission) => permission.label).join('、') }}</span></div></div>
...
.permission-preview-card{margin:0 0 18px;padding:13px 14px;border:1px solid #cce8e9;border-radius:7px;background:#f4fbfb}.permission-preview-card>header{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px}.permission-preview-card>header div{display:flex;flex-direction:column;gap:4px}.permission-preview-card strong{font-size:13px}.permission-preview-card header span{color:var(--muted);font-size:11px}.permission-preview-groups{display:flex;flex-direction:column;gap:6px}.permission-preview-groups>span{color:#53606e;font-size:11px;line-height:1.5}.permission-preview-groups b{display:inline-block;width:58px;color:#078f94;font-weight:600}
</style>
